...
(function (root, factory) {
  ...
  p5.Gen.prototype.triples = function (_x, _args) {
    ...
    for (i in _x) {
      ...
      for (var j = _args.length - 1; j > 0; j -= 3) {
        if (_args[j - 1] != 0.0) {
          var val;
          if (_args[j - 2] == 0.0) val = 1.0;
          // BGG: harmonic 0 (DC)
          else {
            val = sin(
              TWO_PI * (_x[i] / (1.0 / _args[j - 2]) + _args[j] / 360.0)
            );
          }
  ...
  p5.Gen.prototype.bpf = function (_x, _args) {
    ...
    for (i in _x) {
      for (var k = 1; k < _args.length; k += 2) {
        ...
        if (nexttime - thistime < 0.0) {
          // okay for them to be the same
  ...
  p5.Gen.prototype.random = function (_x, _type) {
    ...
    if (!_x) {
      // no arguments, so do linear with random seed
      _type = 'linear';
      _x = [millis()];
      u = false;
    } else if (typeof _x != 'string') {
      // first argument is a number, so seed
      if (
  ...
  p5.Gen.prototype.window = function (_x, _type, _args) {
    ...
    switch (_type) {
      ...
      case 'kaiser':
        ...
        for (i in _x) {
          var above =
            PI * alpha * sqrt(1.0 - (2 * _x[i] - 1.0) * (2 * _x[i] - 1.0));
          var below = PI * alpha;
      ...
      case 'slidinggaussian':
        ...
        var sigma = _args[1] * 2;
        for (var i in _x)
          _y[i] = exp(0.0 - sq(_x[i] * 2 - 1.0 - dx) / (2.0 * sigma * sigma));
        break;
      ...
      case 'hyperelliptic':
        ...
        for (i in _x) {
          ...
          if (_x[i] <= _a) {
            _y[i] =
              (1.0 / _a) * pow(pow(_a, pwn) - pow(_x[i] - _a, pwn), 1.0 / pwn);
          } else {
            _y[i] =
              (1.0 / (1 - _a)) *
              pow(pow(1.0 - _a, pwn) - pow(_x[i] - _a, pwn), 1.0 / pwn);
          }
        }
        break;
      ...
      case 'squircular':
        ...
        for (i in _x) {
          ...
          if (_x[i] <= _a) {
            _y[i] =
              1 - _a + pow(pow(_a, pwn) - pow(_x[i] - _a, pwn), 1.0 / pwn);
          } else {
            _y[i] =
              _a + pow(pow(1.0 - _a, pwn) - pow(_x[i] - _a, pwn), 1.0 / pwn);
          }
        }
        break;
      ...
      case 'poissonhann':
        if (!_args[0]) _args[0] = 0.5; // default center
        var tau =
          25.0 * max(_args[0] * _args[0] * _args[0] * _args[0], Number.EPSILON); // nice control
        for (i in _x) {
      ...
      case 'slidingpoissonhann':
        ...
        var tau =
          25.0 * max(_args[1] * _args[1] * _args[1] * _args[1], Number.EPSILON); // nice control
        for (i in _x) {
  ...
  p5.Ease.prototype.cubicBezier = function (_x, _a, _b, _c, _d) {
    ...
    //-------------------------------------------
    var y0a = 0.0; // initial y
    var x0a = 0.0; // initial x
    ...
    for (var i = 0; i < nRefinementIterations; i++) {
      var currentx =
        ...
        D;
      var currentslope =
        1.0 / (3.0 * A * currentt * currentt + 2.0 * B * currentt + C);
